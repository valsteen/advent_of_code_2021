...
fn main() {
    ...
    let (position_1, position_2) = distances
        .iter()
        .flatten()
        .minmax_by_key(|p| p.0 * p.0 + p.1 * p.1 + p.2 * p.2)
        .into_option()
        .unwrap();
    println!(
        "{}",
        (position_1.0 - position_2.0).abs()
            + (position_1.1 - position_2.1).abs()
            + (position_1.2 - position_2.2).abs()
    );
}
