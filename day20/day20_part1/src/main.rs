...
fn zone(input: &[Vec<bool>], x: i32, y: i32, default: usize) -> usize {
    (-1..=1).map(|dy| (-1..=1).map(move |dx| get_at(input, x, y, dx, dy, default))).flatten().fold(0, |acc, bit| {
        (acc << 1) + bit
    })
}
...
fn enhance_image(image: &[Vec<bool>], enhancement: &[bool], default: usize) -> Vec<Vec<bool>> {
    (-1..=image[0].len() as i32).map(|y| (-1..=image.len() as i32).map(|x| {
        enhance(image, enhancement, x, y, default)
    }).collect()).collect()
}

#[allow(dead_code)]
fn display(image: &[Vec<bool>]) -> String {
    (0..image[0].len()).map(|y| (0..image.len()).map(|x| {
        if image[y][x] { "#" } else { "." }
    }).collect::<String>()).join("\n")
}
...
fn main() {
    ...
    let default = next_default(default, &enhancement);
    let new_image = enhance_image(&new_image, &enhancement, default);
